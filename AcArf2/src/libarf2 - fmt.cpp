...
// Caches
extern const float DSIN[901];
extern const float DCOS[901];
extern const float ESIN[1001];
extern const float ECOS[1001];
...
static inline void GetSINCOS(const double degree)
{
	if (degree >= 0)
	{
		uint64_t deg = (uint64_t)(degree * 10.0);
		deg = (deg > 3600) ? mod_degree(deg) : deg;
		if (deg > 1800)
		{
			if (deg > 2700)
			{
...
static inline bool has_touch_near(const uint64_t hint)
{
	...
	uint64_t u;
	u = hint & 0x1fff;
	float dx = ((int16_t)u - 3072) * 0.0078125f * xscale;
	u = (hint >> 13) & 0xfff;
	float dy = ((int16_t)u - 1536) * 0.0078125f * yscale;

	// Camera transformation integrated.
	float posx = 8.0f + dx * rotcos - dy * rotsin + xdelta;
	float posy = 4.0f + dx * rotsin + dy * rotcos + ydelta;

	float hint_l = posx * 112.5f - 168.75f;
	float hint_r = hint_l + 337.5f;
	float hint_d = posy * 112.5f - 78.75f;
	float hint_u = hint_d + 337.5f;
...
static inline bool is_safe_to_anmitsu(const uint64_t hint)
{
	...
	u = hint & 0x1fff;
	int16_t hint_l = (int16_t)u - 384;
	int16_t hint_r = (int16_t)u + 384;
	u = (hint >> 13) & 0xfff;
	int16_t hint_d = (int16_t)u - 384;
	int16_t hint_u = (int16_t)u + 384;
...
enum
{
	HINT_NONJUDGED_NONLIT = 0,
	HINT_NONJUDGED_LIT = 1,
	HINT_JUDGED = 10,
	HINT_JUDGED_LIT,
	HINT_SWEEPED
};
static inline uint8_t HStatus(uint64_t Hint)
{
	...
	{
		if (TAG)
			return HINT_NONJUDGED_LIT;
		else
			return HINT_NONJUDGED_NONLIT;
	}
}
...
static inline int UpdateArf(lua_State *L)
{
	...
	double dt1;
	auto dts1 = Arf->dts_layer1();
	auto dts1_last = dts1->size() - 1;
	while (!dt1)
	{
		if (dt_p1 >= dts1_last)
		{ // "==" is replaced by ">=" to enhance the robustness.
			uint64_t last_deltanode = dts1->Get(dts1_last);
			uint64_t u;
			u = (last_deltanode >> 32) & 0x3ffff;
			uint32_t init_ms = u * 2;
			if (init_ms <= mstime)
			{
				u = last_deltanode & 0xffffffff;
				double base = (double)u / 100000.0;
				u = last_deltanode >> 50;
				double ratio = (double)u / 100000.0;
				dt1 = base + ratio * (mstime - init_ms);
				break;
			}
			else
				dt_p1--;
		}
		while (dt_p1 < dts1_last)
		{ // ++ or --? Not determined this time.
			uint64_t u;

			uint64_t next_deltanode = dts1->Get(dt_p1 + 1);
			u = (next_deltanode >> 32) & 0x3ffff;
			uint32_t next_init_ms = u * 2;
			if (mstime >= next_init_ms)
			{
				dt_p1++;
				continue;
			}
			...
			u = (current_deltanode >> 32) & 0x3ffff;
			uint32_t current_init_ms = u * 2;
			if (mstime < current_init_ms)
			{
				dt_p1--;
				continue;
			}

			u = next_deltanode & 0xffffffff;
			double next_base = (double)u / 100000.0;
			u = current_deltanode & 0xffffffff;
			double current_base = (double)u / 100000.0;
			u = current_deltanode >> 50;
			double current_ratio = (double)u / 100000.0;
			if (current_base > next_base)
				dt1 = current_base - current_ratio * (mstime - current_init_ms);
			else
				dt1 = current_base + current_ratio * (mstime - current_init_ms);
			break;
		}
	...
	double dt2;
	auto dts2 = Arf->dts_layer2();
	auto dts2_last = dts2->size() - 1;
	while (!dt2)
	{
		if (dt_p2 >= dts2_last)
		{
			uint64_t last_deltanode = dts2->Get(dts2_last);
			uint64_t u;
			u = (last_deltanode >> 32) & 0x3ffff;
			uint32_t init_ms = u * 2;
			if (init_ms <= mstime)
		...
		while (dt_p2 < dts2_last)
		{
			...
			double current_ratio = (double)u / 100000.0;
			if (current_base > next_base)
				dt2 = current_base - current_ratio * (mstime - current_init_ms);
			else
				dt2 = current_base + current_ratio * (mstime - current_init_ms);
			break;
		}
...
{
	for (uint8_t i = 0; i < 10; i++)
		T[i] = dmScript::CheckVector3(L, i + 1);
	return 0;
}
...
static inline int JudgeArf(lua_State *L)
{
	...
	auto hint = Arf->mutable_hint();
	auto idx_size = Arf->index()->size();
	uint64_t _group = (uint64_t)mstime >> 9;
	uint16_t which_group = _group > 1 ? _group - 1 : 0;
	_group = which_group + 3;
	uint16_t byd1_group = _group < idx_size ? _group : idx_size;
	...
	if (any_pressed)
	{
		...
		for (; which_group < byd1_group; which_group++)
		{
			...
			for (uint16_t i = 0; i < how_many_hints; i++)
			{
				...
				{ // HINT_NONJUDGED
					if (htn)
					{ // if we have touch(es) near the Hint,
						if (dt >= -100.0f && dt <= 100.0f)
						{ // we try to judge the Hint if dt[-100ms,100ms].
	...
	else
	{
		for (; which_group < byd1_group; which_group++)
		{
			...
			for (uint16_t i = 0; i < how_many_hints; i++)
			{
				...
				{ // HINT_NONJUDGED
					if (htn)
						hint->Mutate(current_hint_id, (current_hint << 1) >> 1 + 0x8000000000000000);
					else
						hint->Mutate(current_hint_id, (current_hint << 1) >> 1);
				}
				else if (ch_status == HINT_JUDGED_LIT && (!htn))
...
static inline int SetXScale(lua_State *L)
{
	xscale = luaL_checknumber(L, 1);
	return 0;
}
static inline int SetYScale(lua_State *L)
{
	yscale = luaL_checknumber(L, 1);
	return 0;
}
static inline int SetXDelta(lua_State *L)
{
	xdelta = luaL_checknumber(L, 1);
	return 0;
}
static inline int SetYDelta(lua_State *L)
{
...
static const luaL_reg M[] =
	{
		{"InitArf", InitArf}, {"UpdateArf", UpdateArf}, {"FinalArf", FinalArf}, {"SetTouches", SetTouches}, {"JudgeArf", JudgeArf}, // {"JudgeArfController", JudgeArfController},
		{"SetXScale", SetXScale},
		{"SetYScale", SetYScale},
		{"SetXDelta", SetXDelta},
		{"SetYDelta", SetYDelta},
